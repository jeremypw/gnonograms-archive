...
static gboolean gnonogram_cellgrid_leave_grid (Gnonogram_CellGrid* self, GdkEventCrossing* e);
static gboolean _gnonogram_cellgrid_leave_grid_gtk_widget_leave_notify_event (GtkWidget* _sender, GdkEventCrossing* event, gpointer self);
...
static gboolean _gnonogram_cellgrid_leave_grid_gtk_widget_leave_notify_event (GtkWidget* _sender, GdkEventCrossing* event, gpointer self) {
	gboolean result;
	result = gnonogram_cellgrid_leave_grid (self, event);
	return result;
}
...
Gnonogram_CellGrid* gnonogram_cellgrid_construct (GType object_type, gint r, gint c) {
	...
	self->priv->_rows = r;
	self->priv->_cols = c;
	gtk_widget_add_events (GTK_WIDGET (self), (gint) (((((GDK_BUTTON_PRESS_MASK | GDK_BUTTON_RELEASE_MASK) | GDK_POINTER_MOTION_MASK) | GDK_KEY_PRESS_MASK) | GDK_KEY_RELEASE_MASK) | GDK_LEAVE_NOTIFY_MASK));
	...
	g_signal_connect_object (GTK_WIDGET (self), "leave-notify-event", (GCallback) _gnonogram_cellgrid_leave_grid_gtk_widget_leave_notify_event, self, 0);
	return self;
}
...
void gnonogram_cellgrid_draw_cell (Gnonogram_CellGrid* self, Cell* cell, GameState gs, gboolean highlight) {
	...
	cs = (gint) (*cell).state;
	_tmp3_ = gdk_cairo_create (GDK_DRAWABLE (GTK_WIDGET (self)->window));
	_cr = _tmp3_;
	if (cs == CELL_STATE_UNKNOWN) {
		GtkStyle* _tmp4_ = NULL;
		_tmp4_ = gtk_widget_get_style (GTK_WIDGET (self));
		self->priv->cr_color = _tmp4_->bg[GTK_STATE_NORMAL];
	} else {
...
static gboolean gnonogram_cellgrid_leave_grid (Gnonogram_CellGrid* self, GdkEventCrossing* e) {
	gboolean result = FALSE;
	g_return_val_if_fail (IS_GNONOGRAM_CELLGRID (self), FALSE);
	g_signal_emit_by_name (self, "cursor-moved", -1, -1);
	result = FALSE;
	return result;
}
