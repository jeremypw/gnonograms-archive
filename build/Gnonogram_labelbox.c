...
void gnonogram_labelbox_update_label (Gnonogram_LabelBox* self, gint idx, const gchar* txt) {
	...
	_tmp0_ = g_strconcat (self->priv->_attribstart, txt, NULL);
	_tmp1_ = g_strconcat (_tmp0_, self->priv->_attribend, NULL);
	gnonogram_label_set_markup (self->priv->_labels[idx], _tmp1_);
	_g_free0 (_tmp1_);
	_g_free0 (_tmp0_);
}
