...
void resource_init (const gchar* arg0) {
	...
	_tmp26_ = config_get_instance ();
	_tmp27_ = _tmp26_;
	_tmp28_ = g_strconcat (resource_resource_dir, "/games", NULL);
	_tmp29_ = config_get_game_dir (_tmp27_, _tmp28_);
	_tmp30_ = _tmp29_;
	_g_free0 (resource_game_dir);
	resource_game_dir = _tmp30_;
	_g_free0 (_tmp28_);
	_config_unref0 (_tmp27_);
	_tmp31_ = config_get_instance ();
	_tmp32_ = _tmp31_;
	_tmp33_ = config_get_game_name (_tmp32_, RESOURCE_DEFAULTGAMENAME);
	_tmp34_ = _tmp33_;
	_g_free0 (resource_game_name);
	resource_game_name = _tmp34_;
	_config_unref0 (_tmp32_);
	_tmp35_ = g_new0 (GdkColor, 2 * 4);
	_tmp36_ = _tmp35_;
	resource_colors = (g_free (resource_colors), NULL);
	resource_colors_length1 = 2;
	resource_colors_length2 = 4;
	resource_colors = _tmp36_;
	setting = (gint) GAME_STATE_SETTING;
	gdk_color_parse ("GREY", &_tmp37_);
	resource_colors[(setting * resource_colors_length2) + ((gint) CELL_STATE_UNKNOWN)] = _tmp37_;
	gdk_color_parse ("WHITE", &_tmp38_);
	resource_colors[(setting * resource_colors_length2) + ((gint) CELL_STATE_EMPTY)] = _tmp38_;
	gdk_color_parse ("BLACK", &_tmp39_);
	resource_colors[(setting * resource_colors_length2) + ((gint) CELL_STATE_FILLED)] = _tmp39_;
	gdk_color_parse ("RED", &_tmp40_);
	resource_colors[(setting * resource_colors_length2) + ((gint) CELL_STATE_ERROR)] = _tmp40_;
	solving = (gint) GAME_STATE_SOLVING;
	gdk_color_parse ("GREY", &_tmp41_);
	resource_colors[(solving * resource_colors_length2) + ((gint) CELL_STATE_UNKNOWN)] = _tmp41_;
	gdk_color_parse ("YELLOW", &_tmp42_);
	resource_colors[(solving * resource_colors_length2) + ((gint) CELL_STATE_EMPTY)] = _tmp42_;
	gdk_color_parse ("BLUE", &_tmp43_);
	resource_colors[(solving * resource_colors_length2) + ((gint) CELL_STATE_FILLED)] = _tmp43_;
	gdk_color_parse ("RED", &_tmp44_);
	resource_colors[(solving * resource_colors_length2) + ((gint) CELL_STATE_ERROR)] = _tmp44_;
	_tmp45_ = config_get_instance ();
	_tmp46_ = _tmp45_;
	_tmp48_ = config_get_colors (_tmp46_, &_tmp47_);
	config_colors = (_tmp49_ = _tmp48_, _config_unref0 (_tmp46_), _tmp49_);
	config_colors_length1 = _tmp47_;
	_config_colors_size_ = _tmp47_;
	gdk_color_parse (config_colors[0], &_tmp50_);
	resource_colors[(setting * resource_colors_length2) + ((gint) CELL_STATE_EMPTY)] = _tmp50_;
	gdk_color_parse (config_colors[1], &_tmp51_);
	resource_colors[(setting * resource_colors_length2) + ((gint) CELL_STATE_FILLED)] = _tmp51_;
	gdk_color_parse (config_colors[2], &_tmp52_);
