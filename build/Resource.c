...
void resource_init (const gchar* arg0) {
	...
	_tmp37_ = g_strconcat (resource_resource_dir, "/Manual", NULL);
	_g_free0 (resource_manual_dir);
	resource_manual_dir = _tmp37_;
	_tmp38_ = config_get_instance ();
	_tmp39_ = _tmp38_;
	_tmp40_ = g_strconcat (resource_resource_dir, "/games", NULL);
	_tmp41_ = config_get_game_dir (_tmp39_, _tmp40_);
	_tmp42_ = _tmp41_;
	_g_free0 (resource_game_dir);
	resource_game_dir = _tmp42_;
	_g_free0 (_tmp40_);
	_config_unref0 (_tmp39_);
	_tmp43_ = config_get_instance ();
	_tmp44_ = _tmp43_;
	_tmp45_ = config_get_game_name (_tmp44_, RESOURCE_DEFAULTGAMENAME);
	_tmp46_ = _tmp45_;
	_g_free0 (resource_game_name);
	resource_game_name = _tmp46_;
	_config_unref0 (_tmp44_);
	_tmp47_ = g_new0 (GdkColor, 2 * 4);
	_tmp48_ = _tmp47_;
	resource_colors = (g_free (resource_colors), NULL);
	resource_colors_length1 = 2;
	resource_colors_length2 = 4;
	resource_colors = _tmp48_;
	setting = (gint) GAME_STATE_SETTING;
	gdk_color_parse ("GREY", &_tmp49_);
	resource_colors[(setting * resource_colors_length2) + ((gint) CELL_STATE_UNKNOWN)] = _tmp49_;
	gdk_color_parse ("WHITE", &_tmp50_);
	resource_colors[(setting * resource_colors_length2) + ((gint) CELL_STATE_EMPTY)] = _tmp50_;
	gdk_color_parse ("BLACK", &_tmp51_);
	resource_colors[(setting * resource_colors_length2) + ((gint) CELL_STATE_FILLED)] = _tmp51_;
	gdk_color_parse ("RED", &_tmp52_);
	resource_colors[(setting * resource_colors_length2) + ((gint) CELL_STATE_ERROR)] = _tmp52_;
	solving = (gint) GAME_STATE_SOLVING;
	gdk_color_parse ("GREY", &_tmp53_);
	resource_colors[(solving * resource_colors_length2) + ((gint) CELL_STATE_UNKNOWN)] = _tmp53_;
	gdk_color_parse ("YELLOW", &_tmp54_);
	resource_colors[(solving * resource_colors_length2) + ((gint) CELL_STATE_EMPTY)] = _tmp54_;
	gdk_color_parse ("BLUE", &_tmp55_);
	resource_colors[(solving * resource_colors_length2) + ((gint) CELL_STATE_FILLED)] = _tmp55_;
	gdk_color_parse ("RED", &_tmp56_);
	resource_colors[(solving * resource_colors_length2) + ((gint) CELL_STATE_ERROR)] = _tmp56_;
	_tmp57_ = config_get_instance ();
	_tmp58_ = _tmp57_;
	_tmp60_ = config_get_colors (_tmp58_, &_tmp59_);
	config_colors = (_tmp61_ = _tmp60_, _config_unref0 (_tmp58_), _tmp61_);
	config_colors_length1 = _tmp59_;
	_config_colors_size_ = _tmp59_;
	gdk_color_parse (config_colors[0], &_tmp62_);
	resource_colors[(setting * resource_colors_length2) + ((gint) CELL_STATE_EMPTY)] = _tmp62_;
	gdk_color_parse (config_colors[1], &_tmp63_);
	resource_colors[(setting * resource_colors_length2) + ((gint) CELL_STATE_FILLED)] = _tmp63_;
	gdk_color_parse (config_colors[2], &_tmp64_);
	resource_colors[(solving * resource_colors_length2) + ((gint) CELL_STATE_EMPTY)] = _tmp64_;
	gdk_color_parse (config_colors[3], &_tmp65_);
	resource_colors[(solving * resource_colors_length2) + ((gint) CELL_STATE_FILLED)] = _tmp65_;
	_tmp66_ = g_strdup ("Ariel");
	_tmp67_ = _tmp66_;
	_g_free0 (resource_font_desc);
	resource_font_desc = _tmp67_;
	_tmp68_ = g_new0 (gdouble, 2);
	_tmp68_[0] = 0.5;
	_tmp68_[1] = 3.0;
	_tmp69_ = _tmp68_;
	resource_MINORGRIDDASH = (g_free (resource_MINORGRIDDASH), NULL);
	resource_MINORGRIDDASH_length1 = 2;
	resource_MINORGRIDDASH = _tmp69_;
