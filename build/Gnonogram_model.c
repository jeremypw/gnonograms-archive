...
gchar* gnonogram_model_to_hexstring (Gnonogram_model* self);
...
static void gnonogram_model_fill_region (Gnonogram_model* self, gint size, CellState** _arr, int* _arr_length1, gint grade, gint e, gint maxb, gint maxp);
	...
	_arr_size_ = self->priv->_cols;
	{
		...
		r = 0;
		{
			...
			while (TRUE) {
				...
				my2_dcell_array_get_row (self->priv->_display_data, r, &arr, &arr_length1, 0);
				_tmp3_ = utils_hex_string_from_cellstate_array (arr, arr_length1);
				_tmp4_ = _tmp3_;
				g_string_append (sb, _tmp4_);
				_g_free0 (_tmp4_);
				g_string_append (sb, "\n");
...
void gnonogram_model_fill_random (Gnonogram_model* self, gint grade) {
	...
	mincdf = 2 + ((gint) ((self->priv->_rows * grade) / (resource_MAXGRADE * 3)));
	minrdf = 2 + ((gint) ((self->priv->_cols * grade) / (resource_MAXGRADE * 3)));
	{
		gint e;
		e = 0;
		{
	...
	maxb = 1 + ((gint) (self->priv->_cols * (1.0 - (grade / 15.0))));
	{
		gint r;
		r = 0;
		{
			...
			while (TRUE) {
				...
				_tmp2_ = abs (r - midcol);
				gnonogram_model_fill_region (self, self->priv->_cols, &self->_arr, &self->_arr_length1, grade, _tmp2_, maxb, self->priv->_cols);
				my2_dcell_array_set_row (self->priv->_solution_data, r, self->_arr, self->_arr_length1, 0);
			}
		}
	}
	maxb = 1 + ((gint) (self->priv->_rows * (1.0 - (grade / 15.0))));
	{
		gint c;
		c = 0;
		{
			...
			while (TRUE) {
				...
				_tmp4_ = abs (c - midrow);
				gnonogram_model_fill_region (self, self->priv->_rows, &self->_arr, &self->_arr_length1, grade, _tmp4_, maxb, self->priv->_rows);
