 ...
 public class My2DCellArray
{
	...
	public void get_row(int row, ref CellState[] sa, int start=0, int end =_cols-1)
	{
		if (end<0) end=_cols-1;
		for (int c=start;c<=end;c++) sa[c]=_data[row,c];
	}
	
	public void set_row(int row, CellState[] sa, int start=0, int end =_cols-1)
	{
		if (end<0) end=_cols-1;
		for (int c=start;c<=end;c++) _data[row,c]=sa[c];
	}
	
	public void get_col(int col, ref CellState[] sa, int start=0, int end=_rows-1)
	{
		if (end<0) end=_rows-1;
		for (int r=start; r<=end; r++) {sa[r]=_data[r,col];}
	}
	
	public void set_col(int col, CellState[] sa, int start=0, int end=_rows-1)
	{
		if (end<0) end=_rows-1;
		for (int r=start;r<=end;r++) {_data[r,col]=sa[r];}
	}
	
	public void get_array(int idx, bool iscolumn, ref CellState[] sa)
	{
		if (iscolumn) get_col(idx, ref sa);
		else get_row(idx, ref sa);
	}
	
	public void get_region(int idx, bool iscolumn, ref CellState[] sa, int start=0, int end=-1)
	{
		if (iscolumn) get_col(idx, ref sa, start, end);
		else 	get_row(idx, ref sa, start, end);
	}
	
	public void set_region(int idx, bool iscolumn, CellState[] sa, int start=0, int end=-1)
	{
		if (iscolumn) set_col(idx, sa, start, end);
		else set_row(idx, sa, start, end);
	}
