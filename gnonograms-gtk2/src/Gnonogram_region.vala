...
//  private int[] tempStatus2;
  ...
  public Gnonogram_region (My2DCellArray grid){
    ...
    status=new CellState[maxlen]; statusStore=new CellState[maxlen];
  ...
  public bool solve(bool debug, bool hint){
    ...
    * */
    ...
    if(isCompleted) return false;
    int count=0;  bool madechanges=false;
    ...
    if (count==MAXCYCLES){inError=false; stdout.printf("Excessive looping in region "+index.to_string());}
    return madechanges;
  }
  ...
  private bool filledSubregionAudit(){
    ...
    while (currentIdx<nCells){ //find a filled sub-region
      ...
      if(currentIdx==0 || status[currentIdx-1]==CellState.EMPTY) startcapped=true; //edge cell
      ...
      if (lastcell==nCells-1 || status[lastcell+1]==CellState.EMPTY) endcapped=true; //last cell is at edge
  ...
  private bool fillGaps(){
    ...
    for (int idx=0; idx<nCells-2; idx++){
      if (status[idx]!=CellState.FILLED) continue; //find a FILLED cell
      if (status[idx+1]!=CellState.UNKNOWN) continue; //is following cell empty?
      if (!oneowner(idx)){  // if owner ambiguous, can only deal with single cell gap
        ...
        if(status[idx+2]!=CellState.FILLED) continue; //cell after that filled?
        ...
        int blength = countNextState(CellState.FILLED,idx+2,true)
                    + countNextState(CellState.FILLED,idx,false)
                    + 1;
        ...
        // see if setting empty would create two regions one or more of which is too small for the available blocks
          ...
          //stdout.printf("Left "+lengthLeft+" Right "+lengthRight+"\n");
          //int firstBlock=-1;
          //int tmpLength=0;
          ...
          //stdout.printf("must not be empty "+mustNotBeEmpty+"\n");
        ...
        idx++;
        //skip to end of gap
        while (idx<nCells-1 && status[idx]==CellState.UNKNOWN) idx++;

        if (status[idx]!=CellState.FILLED) continue; //gap ends with empty cell - abandon this gap
  ...
  private bool freecellaudit(){
    ...
    if (freecells==tolocate){ // Set all UNKNOWN as COMPLETE
      for (int i=0;i<nCells;i++)  {
        if (status[i]==CellState.UNKNOWN) setcellcomplete(i);
      }
      ...
      for (int i=0;i<nCells;i++) {
        if (status[i]==CellState.UNKNOWN) setcellempty(i);
        isCompleted=true;
      }
      return true;
    }
  ...
  private bool findEdge(int limit, int direction){
    ...
    for (int i=currentIdx; (i>=0 && i<nCells); i+=loopstep){
      if (status[i]==CellState.EMPTY) continue;
      ...
      if (tags[i,isFinishedPointer]){  //skip to end of finished block
        ...
        //stdout.printf("i is now "+i+ " block is now " + currentBlockNum + "\n");
      }
  ...
  private bool fixblocksinranges(){
    ...
    //int e=countcellstate(CellState.EMPTY);
    ...
    //stdout.printf("Empty count "+e+" Available blocks "+bl+" available ranges "+numberOfAvailableRanges+"\n");
  ...
  private bool availableFilledSubRegionAudit(){
    ...
    //stdout.printf("Available block count is "+nAvailableBlocks+"\n");
    ...
    if (countRegions>countCandidates){inError=true; stdout.printf("too many regions\n");return false;}
...
//======================================================================
  private int skipWhileNotStatus(int cs, int idx, int limit, int direction){
    ...
    //bool test1= idx>=limit;
    if (limit<-1 || limit>nCells) {inError=true; message="Invalid limit"; stdout.printf(message); return 0;}
    if ((direction==Gnonogram_region.FORWARDS) && idx>=limit)  return limit;
    else if ((direction==Gnonogram_region.BACKWARDS) && (idx<=limit)) return limit;
  ...
  private bool checkNumberOfBlocks(){
    ...
    int count=0, idx=0;
    while (idx<nCells){
      while (idx<nCells && status[idx]==CellState.EMPTY) idx++;
      if (idx<nCells) count++;
      else break;
      while (idx<nCells && status[idx]!=CellState.EMPTY) idx++;
    }
  ...
  private int countcappedranges(){
    ...
    int range=0, start=0, length=0, idx=0;
    while (idx<nCells && status[idx]!=CellState.FILLED) idx++; //skip to beginning of first range
    while (idx<nCells){
      length=0; start=idx;
      ranges[range,0]=start;
      ...
      idx++;
      while (idx<nCells && status[idx]!=CellState.FILLED) idx++; //skip to beginning of next range
    }
    return range;
  }
...
//    return Arrays.copyOf(tempInt,count);
  ...
  private bool fixBlockInRange(int block, int start, int length){
    //stdout.printf("Fix Block in Range");
  ...
  private void removeBlockFromRange(int block, int start, int length, int direction){
    ...
    if (invalidData(start,block, length)) {
      inError=true; message="remove block from range: invalid data: start "+
      start.to_string()+" block "+block.to_string()+" length "+length.to_string()+
      "nCells "+nCells.to_string()+" nBlocks "+nBlocks.to_string()+"\n";
    }
    else  {
  ...
  private bool setcellowner(int cell, int owner, bool exclusive, bool canbeempty){
    ...
    if (invalidData(cell,owner)){
      inError=true; message="setcellowner - cell "+cell.to_string()+" invalid data\n";
    }
      ...
      if(!canbeempty) {
        status[cell]=CellState.FILLED; changed=true;
...
//  private bool invalidData(int start){
//    return invalidData(start,0,1);
//  }
//  private bool invalidData(int start, int block){
...
//  }
  ...
  private bool cellFilled(int cell){
    return (status[cell]==CellState.FILLED||status[cell]==CellState.COMPLETED);
  }
  ...
  private bool totalsChanged(){
    ...
    if (unknown!=this.unknown){
      ...
      if (filled+completed>blockTotal) recordError("totals changed","too many filled cells");
         ...
         if (filled+completed<blockTotal) recordError("totals changed","too few filled cells - "+filled.to_string());
    ...
    for (int i=0; i<nCells; i++){
      switch (tempStatus[i]){
        case CellState.EMPTY :
          if (!tags[i,canBeEmptyPointer])recordError("getstatus", "cell "+i.to_string()+" cannot be empty");
          else  status[i]=CellState.EMPTY;
          break;
        case CellState.FILLED :
          //dont overwrite COMPLETE status
          if (status[i]==CellState.EMPTY)recordError("getstatus", "cell "+i.to_string()+" cannot be filled");
  ...
  private void tagsToStatus(){
    for (int i=0;i<nCells; i++){
      if(status[i]==CellState.FILLED && tags[i,isFinishedPointer])status[i]=CellState.COMPLETED;
      if(status[i]!=CellState.UNKNOWN) continue;
...
//  }
  ...
  public string to_string()  {
    ...
    sb.append("\n status before:\n");
    for (int i=0; i<nCells; i++) sb.append((tempStatus[i].to_string()));
    sb.append("\n status now:\n");
    for (int i=0; i<nCells; i++) sb.append((status[i].to_string()));
    ...
    public Gnonogram_permutor? get_permutor(out int start)
    {
        ...
        {
            stdout.printf("ERROR in get permutator - more than one range\n");
            return null;
        }

        start=ranges[0,0];
        var p=new Gnonogram_permutor(ranges[0,1],clue);
        //stdout.printf(@"Permutator from $(this) start $start length $(ranges[0,1]) blocks used $(clue)\n");
        return p;
    }
