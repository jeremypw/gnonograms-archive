...
//	public static string translators;
	...
	public const string SOLVEICONFILENAME="computer.png";
	...
	public const string LOGOFILENAME="gnonograms3-256.png";
	//Icon theme names
	...
	public const string HIDEICONTHEMENAME="";
...
//		stdout.printf("gettext package is %s \n",APP_GETTEXT_PACKAGE);
		...
		resource_dir=installed ? Path.build_filename(root_dir,"share","gnonograms",null) : exec_dir;
		stdout.printf("Resource_dir is %s \n",resource_dir);

		locale_dir=installed ? root_dir+"/share/locale" : Path.build_filename(resource_dir,"locale",null);
		stdout.printf("Locale_dir is %s \n",locale_dir);

		icon_dir=Path.build_filename(resource_dir,"icons",null);

		use_gnome_help=(_OS=="LINUX" && _GNOME_DOC==1);
		html_manual_dir=Path.build_filename(resource_dir,"html",null);

		string gnome_manual_dir=installed ? root_dir+"/share/gnome/help/gnonograms/C" : Path.build_filename(resource_dir,"help","C",null);
...
//	}
	...
	{
		Resource.load_game_dir=Path.build_filename(resource_dir,"games",null);
		Resource.save_game_dir=Environment.get_home_dir();
	}
	public static void get_game_dir(Config config_instance)
	{
		Resource.load_game_dir=config_instance.get_load_game_dir(Path.build_filename(resource_dir,"games",null));
	...
	public static Gdk.Pixbuf? get_icon(Resource.IconID id)
	{
		...
		if (icon_theme!=null) icon=get_theme_icon(icon_themename);
		if (icon==null)
		{
