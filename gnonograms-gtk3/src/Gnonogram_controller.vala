...
public class Gnonogram_controller
{
    ...
    private My2DCellArray startgrid;
    ...
    public Gnonogram_controller(string game_path)
    {
        ...
        _solver=new Gnonogram_solver(this);
    ...
    {   //stdout.printf("create view\n");
        ...
        try{
            _gnonogram_view.set_icon_from_file(Resource.icon_dir+"/Gnonograms3.svg");
        }
        ...
        _gnonogram_view.gethint.connect(this.get_hint);
        ...
        _gnonogram_view.setcolors.connect(()=>{Resource.set_rgbas(); redraw_all();});
        _gnonogram_view.setfont.connect(()=>{
                Resource.set_custom_font(_gnonogram_view);
    ...
    private void check_solved()
    {
        ...
        {
                _timer.stop();
                if (_model.count_errors()==0)
                {
                    Utils.show_info_dialog(_("Congratulations - you have solved the puzzle.\n\n")+get_time_taken());
                }
                else
                {
                    Utils.show_info_dialog(_("Congratulations - you have found an alternative solution.\n\n")+get_time_taken());
                    _model.clear_errors();
                }
                _is_button_down=false;
        }
    ...
    private void redraw_all()
    {
        ...
        if(_gridlinesvisible) _cellgrid.draw_grid();
    }
    ...
    public void set_solution_from_solver(){
        if (_have_solution){
            _model.use_solution();
