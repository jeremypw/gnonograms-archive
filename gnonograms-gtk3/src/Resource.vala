/* Resource handling functions for Gnonograms3
...
//Performace/capability related parameters
    ...
    public static int MAXADVANCEDGRADE=300;
...
//Appearance related parameters
    ...
    public static Gdk.RGBA[,] colors;
    public static Gdk.RGBA default_filled_setting;
    public static Gdk.RGBA default_empty_setting;
    public static Gdk.RGBA default_filled_solving;
    public static Gdk.RGBA default_empty_solving;
...
//File location related parameters
    ...
    public static void init(string arg0)
    {
        ...
        resource_dir=installed ? root_dir+"/share/gnonograms3" : exec_dir;
        ...
        string gnome_manual_dir=installed ? root_dir+"/share/gnome/help/gnonograms3/C" : resource_dir+"/help/C";
...
//        Gdk.RGBA.parse("GREY",out colors[setting,(int)CellState.UNKNOWN]);
//        Gdk.RGBA.parse("WHITE", out colors[setting,(int)CellState.EMPTY]);
//        default_empty_setting=colors[setting,(int)CellState.EMPTY];
//        Gdk.RGBA.parse("BLACK", out colors[setting,(int)CellState.FILLED]);
//        default_filled_setting=colors[setting,(int)CellState.FILLED];
//        Gdk.RGBA.parse("RED", out colors[setting,(int)CellState.ERROR]);
...
//        Gdk.RGBA.parse("GREY",out colors[solving,(int)CellState.UNKNOWN]);
//        Gdk.RGBA.parse("YELLOW", out colors[solving,(int)CellState.EMPTY]);
//        default_empty_solving=colors[solving,(int)CellState.EMPTY];
//        Gdk.RGBA.parse("BLUE", out colors[solving,(int)CellState.FILLED]);
//        default_filled_solving=colors[solving,(int)CellState.FILLED];
//        Gdk.RGBA.parse("RED", out colors[solving,(int)CellState.ERROR]);
    ...
    public static void set_rgbas()
    {
        ...
        var label_box=new Box(Gtk.Orientation.VERTICAL,5); label_box.set_homogeneous(false);
        label_box.add(fset_label);
        label_box.add(eset_label);
        label_box.add(fsolve_label);
        label_box.add(esolve_label);

        var filled_setting=new Gtk.ColorButton.with_rgba(colors[(int)GameState.SETTING, (int)CellState.FILLED]);
        filled_setting.title=_("Color of filled cell when setting");
        var empty_setting=new Gtk.ColorButton.with_rgba(colors[(int)GameState.SETTING, (int)CellState.EMPTY]);
        empty_setting.title=_("Color of empty cell when setting");
        var filled_solving=new Gtk.ColorButton.with_rgba(colors[(int)GameState.SOLVING, (int)CellState.FILLED]);
        filled_solving.title=_("Color of filled cell when solving");
        var empty_solving=new Gtk.ColorButton.with_rgba(colors[(int)GameState.SOLVING, (int)CellState.EMPTY]);
        empty_solving.title=_("Color of empty cell when solving");

        //var button_box=new VBox(false,5);
        var button_box=new Box(Gtk.Orientation.VERTICAL,5); button_box.set_homogeneous(false);
        ...
        reset_filled_setting.clicked.connect(()=>{filled_setting.set_rgba(default_filled_setting);});
        ...
        reset_empty_setting.clicked.connect(()=>{empty_setting.set_rgba(default_empty_setting);});
        ...
        reset_filled_solving.clicked.connect(()=>{filled_solving.set_rgba(default_filled_solving);});
        ...
        reset_empty_solving.clicked.connect(()=>{empty_solving.set_rgba(default_empty_solving);});

        //var reset_button_box=new VBox(false,5);
        var reset_button_box=new Box(Gtk.Orientation.VERTICAL,5); reset_button_box.set_homogeneous(false);
        ...
        var vbox= (Gtk.Box)(dialog.get_content_area());
        vbox.add(hbox);
        dialog.show_all();
        if (dialog.run()==ResponseType.OK)
        {
            colors[(int) GameState.SETTING, (int) CellState.FILLED]=filled_setting.get_rgba();
            colors[(int) GameState.SETTING, (int) CellState.EMPTY]=empty_setting.get_rgba();
            colors[(int) GameState.SOLVING, (int) CellState.FILLED]=filled_solving.get_rgba();
            colors[(int) GameState.SOLVING, (int) CellState.EMPTY]=empty_solving.get_rgba();
    ...
    public static void get_colors(Config config_instance)
    {
        ...
        int setting =(int)GameState.SETTING;
        int solving =(int)GameState.SOLVING;
        colors[setting,(int)CellState.EMPTY].parse(config_colors[0]);
        colors[setting,(int)CellState.FILLED].parse(config_colors[1]);
        colors[solving,(int)CellState.EMPTY].parse(config_colors[2]);
        colors[solving,(int)CellState.FILLED].parse(config_colors[3]);


    }

    public static void set_custom_font(Gtk.Window parent)
    {
        //var dialog = new FontSelectionDialog(_("Select font used for the clues"));
        var dialog = new FontChooserDialog(_("Select font used for the clues"),parent);
        if (dialog.run()!=ResponseType.CANCEL)  font_desc=dialog.get_font();
        dialog.destroy();
    }
